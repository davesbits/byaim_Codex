  ...
  async fetch(request, env) {
    ...
    if (env.VECTORIZE_INDEX?.query) {
      try {
        ...
        const chunks = (queryResult.matches || []).map((m) => m.metadata?.text).filter(Boolean).slice(0, 5);
    ...
    const messages = [
      ...
      {
        role: "system",
        content: "Page context: " + page + ". Use the following retrieved notes if they are relevant:\n\n" + (retrievedText || "[no extra notes available yet]")
      },
    ...
    try {
      ...
      answerText = aiRes?.response || aiRes?.output_text || JSON.stringify(aiRes || {});
    } catch (e) {
...
// .wrangler/tmp/bundle-1Yn1dh/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-1Yn1dh/middleware-loader.entry.ts
